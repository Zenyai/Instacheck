...
class Instagram {
    ...
    protected $_endpointUrls = array(
        'authorize' => 'https://api.instagram.com/oauth/authorize/?client_id=%s&redirect_uri=%s&response_type=%s&scope=relationships',
    ...
    */
     ...
     * @param string $text
     */
    public function getUserFollowedBy($text, $cursor = '') {
        $endpointUrl = sprintf($this->_endpointUrls['user_followed_by'], $text, $this->getAccessToken(), $cursor);
        $this->_initHttpClient($endpointUrl);
        return $this->_getHttpClientResponse();
    }
